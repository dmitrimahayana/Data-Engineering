...
from KafkaConsumer_ShowMessages import ShowMessages
...
def infinite_loop_convo(name, run_event):
    counter = 0
...
def take_input(name, run_event):
    while run_event.is_set():
        msg_text = input("Send Message:")
        thread_date = datetime.datetime.now().strftime(dateFormat)
        thread_time = datetime.datetime.now().strftime(timeFormat)
        msg_obj = UserMessages(auth.username, thread_date + "_" + thread_time, msg_text)
...
sm = ShowMessages(auth.username, friends_topic, bootstrap_servers, sr_config, "upwork_user_messages")
sm.config_message()
